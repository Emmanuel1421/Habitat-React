...
import { Caso, TipoAtendimento } from '@/types';
...
const CadastroAtendimento = () => {
  ...
  const estagiarios = mockUsers.filter(u => u.role === 'estagiario' && u.ativo);
  const coordenadores = mockUsers.filter(u => u.role === 'coordenador' && u.ativo);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newCaso: Caso = {
      id: `c${Date.now()}`,
      morador: { nome: form.nome, cpf: form.cpf, telefone: form.telefone, endereco: form.endereco },
      ...
      anotacoes: [],
      documentos: [],
      timeline: [{ id: `t${Date.now()}`, descricao: 'Caso criado', data: now, autor: user?.nome || '', tipo: 'criacao' }],
    };
    addCaso(newCaso);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button type="submit" className="bg-accent text-accent-foreground hover:bg-accent/90">Cadastrar Atendimento</Button>
        </div>
      </form>
    </div>
  );
