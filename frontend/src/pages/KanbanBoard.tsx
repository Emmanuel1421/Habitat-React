...
const KanbanBoard = () => {
  ...
  const handleDrop = (status: CaseStatus) => {
    if (dragId) {
      updateCaso(dragId, { status, dataAtualizacao: new Date().toISOString().split('T')[0] });
      setDragId(null);
    }
  };

  const getUserName = (id: string) => mockUsers.find(u => u.id === id)?.nome || '';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {user?.role === 'master' && (
          <select value={filterEquipe} onChange={e => setFilterEquipe(e.target.value)} className="px-3 py-2 rounded-lg border border-border bg-card text-sm text-foreground">
            ...
            {coordenadores.map(c => <option key={c.id} value={c.id}>Equipe {c.nome}</option>)}
          </select>
        )}
      </div>
